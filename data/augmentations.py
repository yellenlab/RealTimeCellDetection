...
class Normalize(object):
    def __call__(self, image, masks=None, boxes=None, labels=None):
        return image.astype(np.float32)/255, masks, boxes, labels
...
class Pad(object):
    ...
    """
    def __init__(self, width, height, mean=0.5, pad_gt=True):
        self.mean = mean
        self.width = width
        self.height = height
        self.pad_gt = pad_gt
...
class Resize(object):
    ...
    def __init__(self, max_size=300, resize_gt=True, preserve_aspect_ratio=False):
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        if self.resize_gt:
            ...
            #print(boxes[:, [0,2]]) 
            boxes[:, [0, 2]] *= (np.float32(width)  / img_w)
            boxes[:, [1, 3]] *= (np.float32(height) / img_h)
